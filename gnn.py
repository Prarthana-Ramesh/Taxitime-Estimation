...
class TaxiTimeGNN(nn.Module):
    def __init__(self, node_in=2, gnn_hidden=64, pool_emb=64, global_feat_dim=3, mlp_hidden=64):
        ...
        # MLP head after pooling + concat global features
    ...
    def forward(self, data):
        ...
        x = self.act(x)
        # if batch norm -> need correct shape (N_nodes, features)
        x = self.node_bn1(x)
        ...
        x = self.act(x)
        # pooling (per-graph)
        pooled = global_mean_pool(x, batch)  # shape [num_graphs, feature_dim]
        # global_feat
        g = data.global_feat.to(x.dtype)  # shape [num_graphs, 3]
        # concat
